//! Environment discovery for [`super::CloudHandle`]: which TokenFuse Cloud to
//! talk to, and with which admin bearer key.
//!
//! A line-for-line behavioral mirror of the Tauri shell's
//! `apps/desktop/src-tauri/src/money/env.rs` (docs/PHASE1.md wave 3, "both
//! shells behave identically"): that module has zero Tauri-specific
//! dependencies (only `serde`/`std`), so the discovery algorithm itself is
//! reproduced here rather than reinvented. Two sources, tried in order:
//!
...
//!
//! Resolving neither is not an error: [`discover`] returns `None` and
//! [`super::CloudHandle::discover`] fails closed with `CloudError::NoEnvironment`
//! rather than panicking or hanging. This module never touches the network.
...
/// A fully-resolved place to pair a device and read/mutate against. Not
/// itself exported over FFI (the admin bearer must never cross into Swift as
/// a plain value beyond what pairing already consumes it for); `CloudHandle`
/// exposes only [`EnvSource`] and the Cloud URL back out.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn unique_dir(tag: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "genaryx-ffi-cloud-env-test-{tag}-{}-{n}",
            std::process::id()
        ))
    }
