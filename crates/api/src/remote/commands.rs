...
//! and hands its stdout to a dedicated `std::thread` that calls
...
//! `money::commands`' Cloud calls) runs inside `tokio::task::spawn_blocking`,
//! mirroring every sibling panel's identical discipline.
...
/// `bus::feed::EventSink::emit` leaves a failed delivery to the sink rather
/// than the generic reader loop.
pub trait TailSink: Send + Sync + 'static {
...
/// `tokio::task::spawn_blocking(...).await`'s own error type is
/// `tokio::task::JoinError` - this only ever fires on a task panic.
fn join_failed(what: &'static str, e: tokio::task::JoinError) -> RemoteError {
    RemoteError::Internal {
...
async fn ready_client(state: &&RemoteState) -> Result<RemoteClient, RemoteError> {
...
async fn build_status(client: &RemoteClient) -> RemoteStatusDto {
    ...
    let tunnel = tokio::task::spawn_blocking(move || match tunnel_arc.lock() {
...
async fn stop_tail_async(tail: Arc<StdMutex<Option<TailSession>>>) -> Result<(), RemoteError> {
    tokio::task::spawn_blocking(move || stop_tail(&tail))
        .await
        .map_err(|e| join_failed("stop tail", e))?
}

/// Store the freshly spawned tail child, then hand its stdout to a dedicated
/// reader thread that calls [`TailSink::line`] per line and
/// [`TailSink::ended`] once, on EOF/error - see this module's doc comment.
fn spawn_tail_reader<S: TailSink>(
...
pub async fn remote_status(state: &RemoteState) -> Result<RemoteStatusDto, ()> {
    let guard = state.inner.lock().await;
    match &*guard {
...
pub async fn remote_set_environment(
    ...
    let new_ssh = tokio::task::spawn_blocking(move || super::state::pin_ssh_target(ssh_target))
        .await
...
/// reality").
pub async fn remote_wg_connect(state: &RemoteState) -> Result<RemoteStatusDto, RemoteError> {
    let client = ready_client(&state).await?;
    ...
    let tunnel_arc = client.tunnel.clone();
    let bring_up = tokio::task::spawn_blocking(move || {
...
pub async fn remote_wg_disconnect(state: &RemoteState) -> Result<RemoteStatusDto, RemoteError> {
    let client = ready_client(&state).await?;
    let tunnel_arc = client.tunnel.clone();
    tokio::task::spawn_blocking(move || {
        let mut guard = tunnel_arc.lock().map_err(|_| poisoned("tunnel state"))?;
...
pub async fn remote_ssh_check_reachable(state: &RemoteState) -> Result<(), RemoteError> {
    let client = ready_client(&state).await?;
...
/// (docs/PHASE4.md W4 position 4) - lines arrive through `sink`, see this
/// module's doc comment. Generic over `S: TailSink` rather than taking an
/// `AppHandle`: this crate has no shell dependency at all, so the caller
/// (each shell's own wrapper) hands in however IT delivers a line - see
/// [`TailSink`]'s own doc comment.
pub async fn remote_ssh_tail_start<S: TailSink>(
    path: String,
    from_offset: u64,
    sink: S,
    state: &RemoteState,
    ...
    spawn_tail_reader(child, path, client.tail.clone(), sink);

    Ok(build_status(&client).await)
}

/// Stop the in-flight remote tail, if any (always safe to call).
pub async fn remote_ssh_tail_stop(state: &RemoteState) -> Result<RemoteStatusDto, RemoteError> {
    let client = ready_client(&state).await?;
    stop_tail_async(client.tail.clone()).await?;
    Ok(build_status(&client).await)
}
