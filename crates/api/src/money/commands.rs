...
impl From<ConnectorError> for MoneyError {
    fn from(e: ConnectorError) -> Self {
        match e {
            ...
            ConnectorError::InvalidPathSegment(err) => MoneyError::Cloud {
                status: None,
...
async fn ready_client(state: &&MoneyState) -> Result<MoneyClient, MoneyError> {
...
pub async fn money_status(state: &MoneyState) -> Result<MoneyStatusDto, ()> {
    let guard = state.inner.lock().await;
    Ok(match &*guard {
...
pub async fn money_overview(state: &MoneyState) -> Result<OverviewDto, MoneyError> {
...
pub async fn money_savings(state: &MoneyState) -> Result<SavingsDto, MoneyError> {
    let client = ready_client(&state).await?;
