...
- **A desktop shell for Windows/Linux** (Tauri 2) over the same core.
- The **SwiftUI macOS shell** and the mobile **Pocket** pager are built but
  deferred for distribution (Apple Developer account pending); they stay on the
  roadmap, not in the offer.
...
- **The web shell** (`genaryx-web` + `genaryx-api`). The command layer was
  lifted out of the Tauri shell into `genaryx-api`, shared verbatim by desktop
  and web, so the browser console is the same console. Operator auth is one
...
- **Cert broker** (`cert-broker/`): ACME DNS-01 client embedded in the relay
  ...
  desktop/web product needs none of it (WireGuard instead).
...
crates/
  ...
  api/          genaryx-api — the command layer both shells call.
  ...
  ffi/          UniFFI surface for the SwiftUI shell.
apps/
  desktop/      Tauri 2 shell (React + Vite + TS + Tailwind); also builds the web UI bundle.
  macos/        SwiftUI shell (XCFramework over the core). Built, distribution deferred.
...
```sh
cd apps/desktop && npm run build:web     # -> apps/desktop/dist-web
