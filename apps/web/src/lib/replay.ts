 ...
 * forward through). Never throws: with no backend, or on any transport
 * failure, this resolves to an empty list rather than an error - matching
 * `run_events`'s own "never an Err the UI traps on" contract on the Rust side
 * (`crates/api/src/replay.rs`), the same convention `lib/graph.ts`'s
 ...
 * "empty" is the one honest fallback whether or not a backend is configured.
 */
