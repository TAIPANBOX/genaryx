...
/** The honest "nothing to talk to" status this module returns with no
 * backend (a plain `vite build`/browser preview) - mirrors every other
 * panel's `NO_ENVIRONMENT_ERROR`-style guard, never a fabricated enabled
 * state. */
const NO_BACKEND_STATUS: CopilotStatus = {
  enabled: false,
  provider: null,
  model: null,
  endpoint: null,
  local: null,
  disabled_reason: "No backend to talk to.",
};

/** Whole-panel status for the residency banner. Never throws: with no
 * backend (or on any transport failure) it resolves to a renderable disabled
 * status instead - mirrors `lib/identity.ts`'s `fetchIdentityStatus`. */
export async function fetchCopilotStatus(): Promise<CopilotStatus> {
  if (!hasBackend()) return NO_BACKEND_STATUS;
  try {
 ...
 * the Rust side (`crates/api/src/copilot/commands.rs`'s doc comment), not a
 * structured error DTO - `Answer` already derives `Serialize`, so a success
 * crosses the genaryx-web JSON boundary as-is. Callers should render a rejection as
...
export async function askCopilot(question: string): Promise<CopilotAnswer> {
  if (!hasBackend()) throw new Error("No backend to talk to.");
 ...
 * one-round-trip shape as [`askCopilot`], just a different backend command
 * seeded with a fixed, incident-focused prompt built entirely on the Rust
 * side (`crates/api/src/copilot/commands.rs::copilot_explain`) rather than
 * composed here. `incident_id` is snake_case on the wire (the Rust command
 * pins `rename_all = "snake_case"`, matching this app's wire convention -
 * see `money.ts`'s identical note). Same rejection contract as
 * `askCopilot`: throws with no backend and lets any transport rejection propagate as
...
/** Human-readable text for whatever `askCopilot` rejected with. genaryx-web
 * passes a `Result::Err(String)` command's rejection through as that bare string
 * already, so this is normally an identity function; the fallback branches
 * only matter for a transport-level failure. */
 ...
 * pairing (`crates/api/src/evidence/commands.rs`): whether the audit link
 ...
 * (`crates/api/src/copilot/commands.rs::copilot_log_proposal_approved`), so
 ...
 * itself.
 *
 * Never throws: with no backend, or on any transport failure, this resolves to an
...
export async function logProposalApproved(
  ...
  if (!hasBackend()) {
    return { journaled: false, journal_error: "No backend to talk to." };
  }
  try {
