...
/**
 * Renders `value` as a scannable QR code (docs/PHASE5.md W2). The vendored
...
export function QrCode({ value, size = 220 }: { value: string; size?: number }) {
  ...
  // A 4-module quiet zone on every side (the QR spec's minimum) so a phone
  // camera's own edge detection has margin to lock onto the symbol.
  const quiet = 4;
  ...
  return (
    <svg
      ...
      height={size}
      role="img"
      aria-label="QR code: scan with TokenFuse Pocket to pair"
