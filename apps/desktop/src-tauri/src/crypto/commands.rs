...
async fn ready_client(state: &tauri::State<'_, CryptoState>) -> Result<CryptoClient, CryptoError> {
...
#[tauri::command]
pub async fn crypto_status(state: tauri::State<'_, CryptoState>) -> Result<CryptoStatusDto, ()> {
    let guard = state.inner.lock().await;
    Ok(status_dto(&guard))
}
