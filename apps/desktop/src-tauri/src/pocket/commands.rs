    ...
    match relay.devices().await {
        Ok(resp) => {
            ...
            let watch = resp
                ...
                .filter(|d| d.paired)
                .map(to_device_dto);
            let phone_window = resp.window(RelayDeviceKind::Phone).cloned().map(to_window_dto);
            let watch_window = resp.window(RelayDeviceKind::Watch).cloned().map(to_window_dto);
