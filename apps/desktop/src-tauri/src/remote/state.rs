...
//! `tauri::async_runtime::spawn_blocking`, mirroring `memory::commands`'s
//! identical discipline for `EngramClient`, and each cell is a plain
//! `std::sync::Mutex` (not `tokio::sync::Mutex`) for the same reason
//! `memory::state`'s module doc gives: the lock is only ever taken from a
//! blocking OS thread, never held across an `.await`.
//!
