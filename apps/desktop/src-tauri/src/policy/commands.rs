...
async fn ready_client(state: &tauri::State<'_, PolicyState>) -> Result<PolicyClient, PolicyError> {
...
#[tauri::command]
pub async fn policy_status(state: tauri::State<'_, PolicyState>) -> Result<PolicyStatusDto, ()> {
    let guard = state.inner.lock().await;
    Ok(match &*guard {
