...
pub fn discover() -> Option<ResolvedEnv> {
    let dir = taipan_environments_dir()?;
    discover_taipan_in(&dir)
}

/// `~/.taipan/environments`, or `None` when `$HOME` is not set - mirrors
/// `policy::env::taipan_environments_dir`/`money::env::taipan_environments_dir`
/// exactly (same directory; every panel discovers from the same `taipan up`
/// output).
